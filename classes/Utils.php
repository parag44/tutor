	...
	*/
	...
	public function is_tutor_dashboard($subpage = null)
	{
		...
		if (function_exists('is_admin') && is_admin()) {
			...
			global $submenu;
			$query_strings = [];
			$tutor_menu_names = ['tutor', 'edit.php?post_type=courses'];
			$tutor_sub_menus = [];
			foreach ($tutor_menu_names as $tutor_menu_name) {
				$tutor_sub_menus = array_merge($tutor_sub_menus, $submenu[$tutor_menu_name]);
			}
			foreach ($tutor_sub_menus as $tutor_sub_menu) {
				$query_strings[] = $tutor_sub_menu[2];
			}
			$is_tutor_submenu = array_reduce($query_strings, function ($foundMatch, $query_string) {
				return $foundMatch || str_contains($_SERVER['REQUEST_URI'], $query_string);
			});

			return (is_object($screen) && ($screen->parent_base == 'tutor' || $screen->post_type == 'courses' || $screen->post_type == 'course')) || $is_tutor_submenu;
		}
